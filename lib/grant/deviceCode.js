...
module.exports = function activation(options, activate) {
  ...
  function response(txn, res, complete, next) {
    ...
    try {
      ...
      if (arity == 3) {
        // TODO: more closely mirror this fn signatures of core code and token grants?
        activate(txn.deviceCode, txn, activated);
      } else { // arity == 2
        // TODO: should `deviceCode` be a property directly on `txn`.  its not
        //       part of the request.
