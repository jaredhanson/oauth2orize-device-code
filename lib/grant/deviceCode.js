...
module.exports = function activation(options, activate) {
  ...
  function response(txn, res, complete, next) {
    ...
    try {
      ...
      if (arity == 7) {
        activate(txn.client, txn.deviceCode, txn.user, txn.res, txn.req, txn.locals, activated);
      } else if (arity == 6) {
        activate(txn.client, txn.deviceCode, txn.user, txn.res, txn.req, activated);
      } else if (arity == 5) {
        activate(txn.client, txn.deviceCode, txn.user, txn.res, activated);
      } else { // arity == 4
        activate(txn.client, txn.deviceCode, txn.user, activated);
      }
    } catch (ex) {
      return next(ex);
    }
